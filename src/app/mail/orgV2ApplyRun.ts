...
let orgV2ApplyRunDeps: OrgV2ApplyRunDeps | null = null;

export function registerOrgV2ApplyRunDeps(deps: OrgV2ApplyRunDeps): void {
  orgV2ApplyRunDeps = deps;
}

function v2ApplyDeps(): OrgV2ApplyRunDeps {
  if (!orgV2ApplyRunDeps) throw new Error("registerOrgV2ApplyRunDeps not called");
  return orgV2ApplyRunDeps;
}
  ...
  try {
    ...
    if (hadImapChange) {
      ...
      if (state.view === "list" && isTauriRuntime()) {
        try {
          await v2ApplyDeps().loadMailView(false);
        } catch {
          /* ok */
        }
      }
